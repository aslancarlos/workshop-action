#!/bin/bash
# Conjur Secret Retrieval for GitHub Action workshop-action
...
create_pem() {
    ...
    echo "$INPUT_CERTIFICATE" > /workshop-action/conjur_"$INPUT_ACCOUNT".pem
}
...
conjur_authn() {
	...
	if [[ -n "$INPUT_AUTHN_ID" ]]; then
		...
		if [[ -n "$INPUT_CERTIFICATE" ]]; then
            ...
            token=$(curl --cacert "/workshop-action/conjur_$INPUT_ACCOUNT.pem" --request POST "$REST_API_BASE_URI" --header "Content-Type: application/x-www-form-urlencoded" --header "x-cybr-telemetry: $encoded" --header "Accept-Encoding: base64" --data-urlencode "jwt=$JWT_TOKEN")
		else
		...
		if [[ -n "$INPUT_CERTIFICATE" ]]; then
			...
			echo "::debug Authenticating with certificate"
            token=$(curl --cacert "/workshop-action/conjur_$INPUT_ACCOUNT.pem" --data "$INPUT_API_KEY" "$INPUT_URL/authn/$INPUT_ACCOUNT/$hostId/authenticate" --header "Content-Type: application/x-www-form-urlencoded"  --header "x-cybr-telemetry: $encoded" --header "Accept-Encoding: base64")
		else
			# Authenticate and receive session token from Conjur - encode Base64
            echo "::debug Authenticating without certificate"
  ...
  if [[ ${SECRETS[@]} ]]; then
    ...
    for secret in "${SECRETS[@]}"; do
        ...
        if [[ -n "$INPUT_CERTIFICATE" ]]; then
            ...
            secretVal=$(curl --cacert "/workshop-action/conjur_$INPUT_ACCOUNT.pem" -H "Authorization: Token token=\"$token\"" --header "x-cybr-telemetry: $encoded" "$INPUT_URL/secrets/$INPUT_ACCOUNT/variable/$secretId")
        else
